...
```mermaid
graph LR
    Main["Main App<br/>发送配置"]
    Ext["Extension<br/>发送事件"]
...
sequenceDiagram
    ...
    Main->>Ext: 1. 启动时发送 running 消息
    Main->>Ext: 2. 定期发送心跳
    Main->>Ext: 3. 发送 menuConfig，保存快照
    Ext->>Main: 4. 用户点击菜单，发送 click 事件
    Main->>Main: 执行操作
    Ext->>Main: 5. 定期发送心跳（感知主程序存活）
    Main->>Ext: 6. 收到心跳，确认 Extension 在线
```
